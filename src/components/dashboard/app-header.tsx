...
export function AppHeader() {
  ...
  return (
    <>
      ...
      {/* Password change modal — outside header to avoid z-index issues */}
      {showPwd && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50" onClick={() => setShowPwd(false)}>
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            onClick={e => e.stopPropagation()}
            className="bg-card rounded-xl shadow-2xl border w-full max-w-sm mx-4">
